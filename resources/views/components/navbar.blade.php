    ...
    <div class="container mx-auto w-full flex gap-2 items-center">
        ...
        <div class="dropdown dropdown-end" id="user-dropdown">
                ...
                <div class="w-10 rounded-full @guest bg-gray-200 @endguest">
                    @auth
                        @if (Auth::user()->avatar_url !== null)
                            <img src="{{ asset(Auth::user()->avatar_url) }}" alt="User Avatar">
                        @else
                            @php
            ...
            <ul tabindex="0" class="menu menu-sm dropdown-content bg-base-100 rounded-box z-10 mt-3 w-52 p-2 shadow">
                ...
                @endauth
                <li>
                    <p class="justify-between">Dark Mode <span>
...
<div id="mobile-menu"
    class="fixed top-0 left-0 w-64 h-full bg-white dark:bg-gray-900 shadow-lg transform -translate-x-full transition-transform z-30">
    <div class="p-4">
        <label for="menu-toggle" class="btn btn-ghost btn-circle">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                class="w-6 h-6">
